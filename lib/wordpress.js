...
function formatHomeExpertiseTitle(value = "") {
    return stripHtml(value)
        .replace(/[-_\s]+home(?:[-_\s]+new)?$/i, "")
        .replace(/[-_]+/g, " ")
...
function mapHomeExpertiseItems(items = []) {
    return items
        .map((item, index) => {
            ...
            const title = formatHomeExpertiseTitle(image.title || image.altText);

            return {
                id: image.uri || `wordpress-expertise-${index + 1}`,
                imageUrl: image.mediaItemUrl,
                title,
                imageAlt: image.altText || title || "Surgical expertise",
            };
        })
        .filter(Boolean);
}
