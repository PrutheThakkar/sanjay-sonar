...
// WordPress has no mobile-specific overlay field, so each slide's mobile
// text graphic is a locally curated asset matched by banner position.
const HERO_MOBILE_OVERLAY_BY_INDEX = [
    "/images/hero-banner-1-mobile-text.webp",
];
const DEFAULT_HERO_MOBILE_OVERLAY = "/images/35-mob-text.png";
    ...
    return banners
        .map((banner, index) => {
            ...
            return {
                ...
                mobileContent: overlayImage?.mediaItemUrl
                    ? HERO_MOBILE_OVERLAY_BY_INDEX[index] || DEFAULT_HERO_MOBILE_OVERLAY
                    : null,
                contentAlt:
...
export async function getGalleryPageData() {
    ...
    // Uploaded sideways in WordPress; rotated copies are served from /public.
    const rotatedImages = {
        "WhatsApp-Image-2026-07-29-at-7.20.51-PM-1.jpeg": {
            image: "/images/gallery-fiages-aurangabad.jpeg",
            width: 1280,
            height: 720,
        },
    };
    ...
    const mapImage = (node, index, categoryKey) => {
        ...
        const rotated = rotatedImages[node.mediaItemUrl.split("/").pop()];
